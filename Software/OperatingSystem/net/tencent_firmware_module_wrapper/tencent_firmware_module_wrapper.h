...
#define MQTT_MESSAGE_HANDLE_TASK_STACK_SIZE 4096
...
typedef struct tencent_firmware_module_st {
    ...
    //rec->ssid:%s, bssid:%s, channel:%d, rssi:%d
    int (*get_ap_info)(char *ssid,char *bssid,int *channel,int *rssi);
...
/**
 * @brief Get the wifi info
 *
 * @param[in] ssid    the pointer to ssid value
 *			  bssid	  the pointer to bssid value
 *			  channel the pointer to channel value
 * 			   rssi   the pointer to rssi value
 *
 * @return  errcode
 */
int tos_tf_module_get_info(char *ssid,char *bssid,int *channel,int *rssi);
