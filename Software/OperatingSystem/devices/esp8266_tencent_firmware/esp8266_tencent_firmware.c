...
static int esp8266_tencent_firmware_rst(void)
{
	int try = 0;
	    at_echo_t echo;
	    ...
	    while (try++ < 10) {
	        tos_at_cmd_exec_until(AT_AGENT, &echo, 5000, "AT+RST\r\n");
	        if (echo.status == AT_ECHO_STATUS_EXPECT) {
	            return 0;
	        }
	    }
...
static int esp8266_tencent_firmware_init(void)
{
    ...
    if (esp8266_tencent_firmware_rst() != 0) {
...
static int esp8266_tencent_firmware_get_ap_info(char *_ssid,char *_bssid,int *_channel,int *_rssi)
{
    at_echo_t echo;
    int ret;
    ...
    memcpy(_ssid,ssid_buf,strlen(ssid_buf));
    memcpy(_bssid,bssid_buf,strlen(bssid_buf));
    *_channel = channel ;
    *_rssi = res_rssi ;
    return 0;
}
...
tencent_firmware_module_t tencent_firmware_module_esp8266 = {
	...
	.get_ap_info   	    = esp8266_tencent_firmware_get_ap_info,
